...
export async function POST(req: Request) {
  ...
  if (!curriculum_summary?.trim()) {
    return NextResponse.json({ error: "Curriculum summary is required" }, { status: 400 });
  }
  if (!cart_open_date || !cart_close_date || !cohort_start_date) {
    return NextResponse.json({ error: "All dates are required" }, { status: 400 });
  }
  if (cart_close_date <= cart_open_date) {
    return NextResponse.json(
      { error: "Cart close date must be after cart open date" },
      { status: 400 }
    );
  }
  if (cohort_start_date <= cart_close_date) {
    return NextResponse.json(
      { error: "Cohort start date must be after cart close date" },
      { status: 400 }
    );
  }
  ...
  const [cohort] = await db
    ...
    .values({
      ...
      cartOpenDate: cart_open_date,
      cartCloseDate: cart_close_date,
