...
export function DemoClient({ cohort }: { cohort: DemoCohort }) {
  const [activeTab, setActiveTab] = useState<"sequence" | "calendar" | "voice">(
    "sequence"
  );
...
// ─── Jordan's Voice Fingerprint ─────────────────────────────────────────────

const JORDAN_DIMENSIONS = [
  { label: "Conversational", score: 9 },
  { label: "Direct", score: 9 },
  { label: "Empathetic", score: 8 },
  { label: "Urgent", score: 7 },
  { label: "Storytelling", score: 8 },
  { label: "Question-led", score: 9 },
  { label: "We/Us language", score: 8 },
  { label: "Low fluff", score: 9 },
];

function VoiceFingerprintPanel() {
  const n = JORDAN_DIMENSIONS.length;
  const cx = 200;
  const cy = 200;
  const maxR = 150;
  ...
  function ring(fraction: number) {
    return JORDAN_DIMENSIONS.map((_, i) => {
  ...
  const dataPoints = JORDAN_DIMENSIONS.map((d, i) => {
    const angle = (2 * Math.PI * i) / n - Math.PI / 2;
    const r = maxR * (d.score / 10);
    return [cx + r * Math.cos(angle), cy + r * Math.sin(angle)] as [number, number];
  });

  const labelPoints = JORDAN_DIMENSIONS.map((d, i) => {
    const angle = (2 * Math.PI * i) / n - Math.PI / 2;
    const r = maxR + 28;
    return {
      ...
      <div className="bg-white rounded-xl border border-pigeon-warm-rule p-6 flex flex-col items-center">
        <svg viewBox="0 0 400 400" className="w-full max-w-sm">
          ...
          {[0.25, 0.5, 0.75, 1].map((f) => (
            <path key={f} d={toPath(ring(f))} fill="none" stroke="#e5e7eb" strokeWidth="1" />
          ))}
          {/* Spokes */}
          {JORDAN_DIMENSIONS.map((_, i) => {
            const outer = ring(1)[i];
            return <line key={i} x1={cx} y1={cy} x2={outer[0]} y2={outer[1]} stroke="#e5e7eb" strokeWidth="1" />;
          })}
          {/* Data polygon */}
          <path d={toPath(dataPoints)} fill="#2D3282" fillOpacity="0.15" stroke="#2D3282" strokeWidth="2" />
          {/* Data dots */}
          {dataPoints.map(([x, y], i) => (
            <circle key={i} cx={x} cy={y} r="4" fill="#2D3282" />
          ))}
          {/* Labels */}
          {labelPoints.map(({ x, y, label, score }) => (
            <text
              key={label}
              x={x}
              y={y}
            ...
            >
              <tspan x={x} dy="-7">{label}</tspan>
              <tspan x={x} dy="15" fill="#2D3282" fontWeight="700">{score}/10</tspan>
            </text>
          ))}
        </svg>
      </div>
